...
import { useTOCNavigation } from "./helpers/useTOCHooks.jsx";
...
const ContentsChapter = ({ textId, contentId, segmentId, versionId, addChapter, removeChapter, currentChapter, totalChapters, setVersionId }) => {
  ...
  const { fetchContentBySegmentId } = useTOCNavigation(
    textId, contentId, versionId, size, segmentId, infiniteQuery
  );
  ...
  const renderChapter = () => {
    const propsForUseChapterHookComponent = {
      ...
      setVersionId,
      infiniteQuery,
      fetchContentBySegmentId: fetchContentBySegmentId,
