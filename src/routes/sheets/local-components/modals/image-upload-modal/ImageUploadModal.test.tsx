...
vi.mock("../../../../../config/axios-config", () => ({
  default: {
    post: vi
      .fn()
      .mockResolvedValue({ data: { url: "http://test.com/image.jpg" } }),
  },
}));
...
vi.mock("react-icons/fa6", () => ({
  FaCropSimple: ({ onClick }) => (
    <span data-testid="crop-icon" onClick={onClick}>
      ✂️
    </span>
  ),
}));
...
describe("ImageUploadModal", () => {
  ...
  beforeEach(() => {
    ...
    mockUploadImageToS3 = vi
      .spyOn(uploadFunctions, "UploadImageToS3")
      .mockResolvedValue({ url: mockUrl });
    mockUploadProfileImage = vi
      .spyOn(uploadFunctions, "uploadProfileImage")
      .mockResolvedValue({ url: mockUrl });
  ...
  test("displays selected file information and upload button after file drop", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(`Selected: ${mockFile.name}`),
      ).toBeInTheDocument();
    });
