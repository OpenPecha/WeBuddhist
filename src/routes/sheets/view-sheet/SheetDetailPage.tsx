...
export const updateSheetVisibility = async (sheetId: string, isPublished: boolean, sheetData: SheetData): Promise<boolean> => {
...
const SheetDetailPage = ({ addChapter, currentChapter }: SheetDetailPageProps) => {
  ...
  const { mutate: deleteSheetMutation, isLoading: isDeleting } = useMutation<boolean>({
  ...
  const { mutate: updateVisibilityMutation } = useMutation<boolean, unknown, boolean>({
  ...
  const renderSegment = (segment: SheetSegment) => {
    switch (segment.type) {
        ...
        return (
          ...
          >
            <div className="w-full flex rounded border border-dashed border-gray-300 p-6 transition-shadow hover:shadow-sm">
              <div className={`${getLanguageClass(segment.language || 'en')} text-lg border-l-2 border-red-900 px-2 text-left`}>
              <p dangerouslySetInnerHTML={{__html:segment.content}} />
              <p className="text-sm mt-2 font-semibold text-[#A9080E]">{segment.text_title}</p>
              </div>
            </div>
          </button>
        );
      ...
      case 'image':
        return (
          <div className="flex items-center justify-center" key={segment.segment_id}>
            <figure className="m-0 w-full">
              <img src={segment.content} alt="Sheet content" className="h-auto w-full max-h-[600px] rounded object-cover" />
            </figure>
          </div>
        );
      case 'video':
        return (
          <div className="flex w-full items-center justify-center" key={segment.segment_id}>
  ...
  if (isLoading) {
      return <p className=" h-screen w-full flex items-center justify-center text-center text-gray-700">{t("common.loading")}</p>;
  }

  if (!sheetData || sheetData.content.segments.length === 0) {
    return <p className=" h-screen w-full flex items-center justify-center text-center text-gray-700">{t("text_category.message.notfound")}</p>;
  }
      ...
      <div className="flex items-center justify-between border-y border-gray-300 py-3 text-gray-700">
        ...
        <div className="flex items-center space-x-4">
          {sheetData.publisher.email === userInfo?.email && (
            ...
            className={sheetData.is_published ? 'bg-[#DCFCE7] text-green-500 hover:bg-[#DCFCE7]/80 font-bold' : 'bg-white text-gray-500 hover:bg-gray-100 font-bold'}
          ...
          <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                >
                  {isDeleting ? t('sheet.deleting.message') : t('sheet.delete_button')}
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
      ...
      >
        ...
        />
          ...
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-sm font-semibold uppercase text-gray-700">
            {sheetData.publisher.name.split(' ').map((name: string) => name[0]).join('').substring(0, 2).toUpperCase()}
          </div>
        )}
      ...
      <div >
        <section>
          <div className="flex flex-col gap-2">
            {sheetData.content.segments.map((segment: SheetSegment) => renderSegment(segment))}
          </div>
        </section>
      </div>
    )
    ...
    <div className="flex w-full">
      <main className={`flex-1 p-8`}>
        <article className="mx-auto max-w-3xl space-y-4">
          <h1 className=" text-2xl font-semibold text-gray-700">{sheetData.sheet_title}</h1>
          {renderUserInfo()}
          {renderViewToolbar()}
          {renderSheetContent()}
        </article>
...
const SheetDetailPageWithPanelContext = ({ addChapter, currentChapter }: SheetDetailPageProps) => (
  <PanelProvider>
