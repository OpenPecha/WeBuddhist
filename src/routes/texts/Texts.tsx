...
export const fetchTableOfContents = async (textId: string, skip: number, limit: number) => {
...
export const fetchVersions = async (textId: string, skip: number, limit: number) => {
  const language=sessionStorage.getItem('textLanguage');
  const mappedLanguage = language ? mapLanguageCode(language) : "en";
...
export const fetchCommentaries = async (textId: string, skip: number, limit: number) => {
...
const Texts = () => {
  ...
  const breadcrumbItems: BreadcrumbItemType[] = useMemo(() => {
    const items: BreadcrumbItemType[] = [{ label: t('header.text'), path: '/' }];
    ...
    <TwoColumnLayout
      main={
        ...
        <div className="flex w-full flex-col space-y-6 text-left">
          ...
          <Link to={`/chapter?text_id=${textId}&content_id=${tableOfContents?.contents[0]?.id}&versionId=&contentIndex=${0}`}
          className="text-left">
            <h1 className={`text-gray-800 ${getLanguageClass(versions?.text?.language)}`}>
              {versions?.text?.title}
            </h1>
          </Link>
          {renderTabs()}
